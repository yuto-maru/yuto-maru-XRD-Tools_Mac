...
def format_value(value, role):
    if role in ("q (nm–1)", "d-spacing (nm)"):
        # 10未満は小数第2位、10以上は小数第1位で四捨五入する。
        # 数値に変換できない記号や文字列は、そのまま保持する。
        try:
            number = Decimal(value.replace(",", "").strip())
        except (InvalidOperation, AttributeError):
            return value

        decimal_places = 1 if abs(number) >= Decimal("10") else 2
        quantum = Decimal("0.1") if decimal_places == 1 else Decimal("0.01")
...
def clear_all():
    ...
    output_header_label.config(text="    ".join(OUTPUT_HEADERS))
    output_row_count_label.config(text="行数: 0")
...
def generate_output():
    ...
    # ヘッダーはコピー用テキストに含めず、出力欄の見出し横に表示する
    ...
    output_row_count_label.config(text=f"行数: {max_len}")

    status_label.config(text=f"Word貼り付け用表を生成しました（{max_len} 行）")
...
def copy_output():
    ...
    copied_rows = len(output.splitlines())

    root.clipboard_clear()
    root.clipboard_append(output)
    output_row_count_label.config(text=f"行数: {copied_rows}")
    status_label.config(text=f"出力をクリップボードにコピーしました（{copied_rows} 行）")
...
output_heading_frame = tk.Frame(root)
output_heading_frame.pack(fill=tk.X, padx=10, pady=(10, 0))

tk.Label(
    output_heading_frame,
    text="Word貼り付け用",
    font=("TkDefaultFont", 10, "bold")
).pack(side=tk.LEFT)

output_header_label = tk.Label(
    output_heading_frame,
    text="    ".join(OUTPUT_HEADERS),
    anchor="w"
)
output_header_label.pack(side=tk.LEFT, padx=(16, 0))

output_row_count_label = tk.Label(
    output_heading_frame,
    text="行数: 0",
    anchor="e"
)
output_row_count_label.pack(side=tk.RIGHT)
