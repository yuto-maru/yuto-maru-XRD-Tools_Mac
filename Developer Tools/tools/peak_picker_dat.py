...
def main():
    ...
    for sample_file in dat_files:
        ...
        def draw_plot():
            ax.clear()
            # ax.clear()で解除されるため、再描画時にも4:3を維持する。
            ax.set_box_aspect(0.75)
            selected.clear()
            text_objects.clear()
            plotted_artists.clear()
            manual_indices.clear()
            manual_artists.clear()
        ...
        def find_text_for_index(idx):
            for txt, mapped_idx in text_objects.items():
                if mapped_idx == idx:
                    return txt
            return None

        def remove_manual_index(idx):
            artists = manual_artists.pop(idx, None)
            if artists is None:
                return

            marker, txt = artists
            manual_indices.discard(idx)
            selected.discard(idx)
            text_objects.pop(txt, None)
        ...
        def add_manual_index(idx):
            ...
            selected.add(idx)
            manual_artists[idx] = (marker, txt)
            text_objects[txt] = idx

        def on_manual_click(event):
            if not manual_mode["active"]:
                return
